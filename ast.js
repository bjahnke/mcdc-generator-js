class Stack {
  ...
  constructor(a_list = [], stack_pointer_zero = false) {
    ...
    this.get_top_of_stack = Stack._hof_get_top_of_stack(
      this.stack,
      stack_pointer_zero
    );
    this.pop_top_of_stack = Stack._hof_get_top_of_stack(
      this.stack,
      stack_pointer_zero,
      this.stack.pop.bind(this.stack)
    );
  }
  ...
  static _hof_get_top_of_stack(a_list, stack_pointer_zero, apply_func = null) {
    ...
    function try_stack_operation(index) {
      function result_func() {
        try {
          const stack_val = apply_func(index);
          return stack_val;
        } catch (error) {
          if (a_list.length > 0) {
            throw new Error(
              "Brian goofed, index out of bounds for non-empty stack"
            );
          }
          return false;
        }
...
export class Parser {
  ...
  parse_expr() {
    const transition_dict = {
      var: {
        [tT.LPAREN]: { func: this.token_to_ops, "next state": "var" },
        [tT.NOT]: { func: this.token_to_ops, "next state": "var" },
        [tT.SYMBOL]: { func: this.symbol_to_node, "next state": "op" },
      },
      op: {
        [tT.AND]: { func: this.process_ops, "next state": "var" },
        [tT.OR]: { func: this.process_ops, "next state": "var" },
        [tT.RPAREN]: { func: this.rparen_process, "next state": "op" },
      },
    };
...
export function generate_ast(condition_string) {
  /**
   * Generates an abstract syntax tree given the input conditional string,
   * returns the root node of the tree
   */
  const lexed = new Lexer(condition_string);
  const parsed = new Parser(lexed.tokens, lexed.symbols);
  // console.log("Symbol List: " + str(parsed.symbol_list));
  return parsed;
}